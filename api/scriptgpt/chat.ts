...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!adminKey) return res.status(500).json({ error: 'ScriptGPT AI not configured. Please set the Gemini API key in Settings.' });
  ...
  try {
    ...
    if (fullText) {
      await prisma.message.create({ data: { conversationId: conversation.id, role: 'assistant', content: fullText, aiProvider: 'SCRIPTGPT', model: 'gemini-2.5-flash-lite' } });
      await prisma.conversation.update({ where: { id: conversation.id }, data: { updatedAt: new Date() } });
    }
  } catch (error: any) {
    console.error('ScriptGPT error:', error.message);
